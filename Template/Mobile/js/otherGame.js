				...
				if(typeof data[key] !='string'){
					...
					if(issan>-1){
						for(var i=0;i<data[key].length;i++){
                            ...
                            html1+='<li class="specific-cell-o code_value chan" data-hm="'+data[key][i].class3+'" data-name="'+names+'" data-pl="'+data[key][i].rate+'" data-durate="'+data[key][i].rate+'" data-id="'+data[key][i].id+'"style="height:auto;">'+
						...
						for(var i=0;i<data[key].length;i++){
	                        if(data[key][i]==undefined){
	                        	...
	                        	var codeClass=gameaj.filters.isPInt(data[key][i].class3)?"":'chan';
                                 ...
                                 html+='<li class="specific-cell-o code_value '+codeClass+'" data-hm="'+data[key][i].class3+'" data-name="'+names+'" data-pl="'+data[key][i].rate+'" data-durate="'+data[key][i].rate+'" data-id="'+data[key][i].id+'"style="height:auto;">'+
                                '<span valplace="0">'+data[key][i].class3+'</span>'+
                                '<div class="lot-odds">'+data[key][i].rate+'</div>'+
                                '</li>'; 
	                        }
		...
		龙虎斗:function(data){
			...
			for(var key in data){
                ...
                html+=  '<dl>'+
                            ...
                            for(var i=0;i<data[key].length;i++)
                            {
                                html+=  '<li class="specific-cell-o code_value w_'+data[key].length+'" data-hm="'+data[key][i].class3+'" data-pl="'+data[key][i].rate+'" data-durate="'+data[key][i].rate+'" data-name="'+names+'" data-id="'+data[key][i].id+'" style="margin-top:-3px;">'+
    	...
    	整合:function(data){
			...
			for(var key in data){
				...
				if(typeof data[key] !='string'){
                    ...
                    html+='</ul><div style="clear: both; padding:1px 0;  border-bottom: 1px solid #ddd;"></div>';
                    $('#lot_num').html(html);	
                }
...
function setList(isdan){
        ...
        if(isdan){
            ...
            $('.ball-active').each(function(i){
               ...
               html+='<dd class="yBettingList" class="bet-t" data-hm="'+code+'"  data-id="'+id+'" data-pl="'+pl+'" data-mone="'+mone+'" data-count="1" style="height:65px;padding:5px;">'+
                        '<div class="numberBox yBettingDiv">'+
                            '<span class="number">'+ 
                                '<em>'+code+'</em>'+
                            '</span>'+
                            '<span class="number" style="margin-left:15px;">'+ 
                                '<em>赔率：'+pl+'</em>'+'<em  style="position:absolute;top:3px;right:165px;">倍数:</em>'+
                            '</span><div class="bet_odd"><i class="jian">-</i><input class="bei" type="tel" name="bet_odd_num" value="1" onkeypress="return (/[\\d]/.test(String.fromCharCode(event.keyCode)))" >'+
                                   '<i class="jia">+</i></div>'+
                        ...
                        '<div class="yzongRmb" style="float: left;padding-left: 5px;padding-bottom:1px;font-size:15px;">'+
                        '<div class="yzongRmb qian" style="float: left;padding-left: 5px;"> = <span id="mm">'+mone+'</span>元</div></div>'+
    ...
    $(".jia").click(function () {
        var bei = $(this).prevAll('.bei').val();
        var su= Number(bei)+1;
        var total = su*mone;
        $(this).prevAll('.bei').val(su);
        $(this).parents('.yBettingList').find('#mm').html(total);
        var nu = 0;
        $('.yBettingLists').children('.yBettingList').each(function(i){

            var ne=$(this).find('#mm').html();
            nu+=parseInt(ne);
        })
        $('#f_gameOrder_amount').text(nu);
    })
    $('.jian').click(function () {
        var bei = $(this).siblings('.bei').val();
        var su= Number(bei)-1;
        su = su<1?1:su;
        var total = su*mone;
        $(this).siblings('.bei').val(su);
        $(this).parents('.yBettingList').find('#mm').html(total);
        var nu = 0;
        $('.yBettingLists').children('.yBettingList').each(function(i){
            var ne=$(this).find('#mm').html();
