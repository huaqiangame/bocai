$(function () {
  ...
  function lhclmgameNumber(arr,beis){
    var box = $('.g_Number_Section');
    var boxList = $('<div class="selectNumbers lhclm" style="width:100%;padding: 0 12px;overflow: hidden;display:block;margin:0 auto;"></div>');
    var beisArr = beis.split('|');
    if(_thisPlayid == 'tmbb' || _thisPlayid == 'sxtx' ||　_thisPlayid == 'sx1x'){
      for(var j = 0; j < arr.length; j++){
        var tmbbArr = arr[j].split('|');
        boxList.append('<div style="width: 87px;margin: 8px 2px;display: inline-block;float: left;border: 1px solid #dfdfdf;border-radius: 3px;background: #faf9f6;">'+
  ...
  */
  ...
  var dufrate=parseFloat(yrates[_thisPlayid].rate);
  ...
  function silber(name){
    ...
    var arrs=['lm3z2','lm2zt','sx2xl','sx3xl','sx4xl','ws2wl','ws3wl','ws4wl']
    if(isNaN(fands)||arrs.indexOf(name)>-1){
      $('.sliderConter').hide();
      return;
    }else{
      $('.sliderConter').show();
