		...
		if(data.sign==true){
			...
			if (lottery.remainTime && eval(lottery.remainTime) > 1) {
				...
				if(yIndex>1){
					art.dialog({
						time: 2,
						content:lottery.lastFullExpect+'期投注时间已截止',
						lock:true
					});
				}
				yIndex++;
...
var lotteryopencodes = function(lotteryname){
	...
	$.post(url, {'lotteryname':lotteryname}, function(data) {
        ...
        var yopentime = yopentimes_arr[1];
				html += '<tr><td width="130">'+openinfo.expect+'</td><td width="46" class="c_red fb" style="background-image:-webkit-linear-gradient(0deg,#bdb48b,#bdb48b 0.3%,transparent 0);color:red;">'+openinfo.opencode+'</td><td width="40" class="c_blue" style="padding:5px;">'+yopentime+'</td></tr>';
            ...
            if (msg.sign == true) {
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
                        ...
                        var yopentimes_arr = yopentimes.split(' ');
                        var yopentime = yopentimes_arr[1];
                        var html = '<tr><td class="lsqh">' + openexpect + '</td><td style="color:red;background-image:-webkit-linear-gradient(0deg,#bdb48b,#bdb48b 0.3%,transparent 0);" class="c_red fb';
                        html += '">';
                        ...
                        html += '</td><td class="c_blue" style="padding:5px;">'+yopentime+'</td></tr>';
