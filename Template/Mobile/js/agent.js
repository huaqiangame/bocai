		...
		success: function(data) {
			...
			$.each(data, function(idx, val) {
				...
				if ((eval(val.usenum) - eval(val.okusenum)) <= 0) {
					...
					html += '<div class="item-after"><t style="color:red" onclick="viewlink(\'' + registerUrl + val.uid + '\')">查看</t></div>';
				}
				html += '</div>';
...
function viewlink(linkurl){
      $.modal({
            text:'<textarea class="target" style="width:100%;" readonly>'+linkurl+'</textarea>',
            buttons: [
              {text: '<span  data-clipboard-target=".target" id="gb">复制</span> <script>$(\'#gb\').click(function () {var clipboard = new Clipboard(\'#gb\');})</script>'},
          ],
        });
