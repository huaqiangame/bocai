$(function () {
  ...
  //号码点击
  $('.g_Number_Section').on('click','.selectNumbers a,.kl8_qw a',function (){
  ...
  function gameNumberQw(arr){
    ...
    var boxList = $('<div class="kl8_qw"></div>');
  ...
  */
  ...
  $('.single-slider').jRange({
    ...
    onstatechange:function(value,a){
      console.log(value);
            var fands=yrates[_thisPlayid].fandian*10;//parseFloat(yrates['tmzx'].fandian)/100;
            var dufrate=yrates[_thisPlayid].maxjj.indexOf('|')>-1?yrates[_thisPlayid].maxjj:parseFloat(yrates[_thisPlayid].maxjj);
            var fandn=Math.round(dufrate)/100;

            setslide(value,dufrate,fandn,_thisPlayid);
          },
          callback:function(a){
  ...
  function silber(name){
    ...
    console.log(yrates[name].maxjj.indexOf('|'));
    dufrate=yrates[name].maxjj.indexOf('|')>-1?yrates[name].maxjj:parseFloat(yrates[name].maxjj);
