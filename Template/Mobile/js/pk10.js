...
var lotteryopencodes = function(lotteryname){
	...
	$.post(url, {'lotteryname':lotteryname}, function(data) {
        ...
        var yopentime = yopentimes_arr[1];
				html += '<tr><td width="130">'+openinfo.expect+'<br>'+yopentime+'</td><td width="46" class="c_red fb_pk10" style="background-image:-webkit-linear-gradient(0deg,#bdb48b,#bdb48b 0.3%,transparent 0);color:red;">'+openinfo.opencode+'</td></tr>';
            ...
            if (msg.sign == true) {
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
                        ...
                        var yopentimes_arr = yopentimes.split(' ');
                        var yopentime = yopentimes_arr[1];
                        var html = '<tr><td class="lsqh">' + openexpect + '<br>'+yopentime+'</td><td style="color:red;background-image:-webkit-linear-gradient(0deg,#bdb48b,#bdb48b 0.3%,transparent 0);" class="c_red fb_pk10';
                        html += '">';
