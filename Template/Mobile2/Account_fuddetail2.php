    ...
    <section class="ui-container fuddetai_infosCont">
        <div class="ui-tab u-recordcontainer">
            <ul class="ui-tab-nav ui-border-b">
                ...
                <li class="current"><A href="{:U('Account/fuddetail2')}">充值</A></li>
...
<h4><p class="fz14">{$paylist[$vo['paytype']]['title']}(附言码:{$vo.fuyanma})</p><p class="c_8 fz12">{$vo.oddtime|date="m-d H:i:s",###}&nbsp;&nbsp;<if condition="$vo['status'] eq 1"><span style="color:green">成功</span><elseif condition="$vo['status'] eq 0" /><span style="color:grey">支付中</span><elseif condition="$vo['status'] eq -1" /><del style="color:grey">订单关闭</del></if></p></h4>
<div class="ui-txt-info c_blue">金额：{$vo.amount}</div>
</li></volist> 
</ul>
