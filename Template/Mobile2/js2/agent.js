...
function groupReportCur(daytype) {
  if (daytype == undefined) {
    daytype = 1;
  }
  var loginname = $("#groupReportSearchLoginname" + daytype).val();

  var tabs = $("#downuseraccountreportlist" + daytype);
  tabs.empty();
  var url = apirooturl + 'groupreport';
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.sign) {
        ...
        var val = data.root;
        var html = '';
        html += '<li><span>' + val.dayConsumptionMoney + '</span>投注金额</li>';
        html += '<li><span>' + val.dayIncomeMoney + '</span>中奖金额</li>';
        html += '<li><span>' + val.dayActivitiesMoney + '</span>活动礼金</li>';
        html += '<li><span>' + val.dayDividendMoney + '</span>团队盈利</li>';
        html += '<li><span>' + val.dayRechargeMoney + '</span>充值金额</li>';
        html += '<li><span>' + val.dayDrawRechargeMoney + '</span>提现金额</li>';
        html += '<li><span>' + val.regNum + '</span>注册人数</li>';
        html += '<li><span>' + val.subNum + '</span>下级人数</li>';
        html += '<li><span>' + val.groupBalance + '</span>团队余额</li>';
