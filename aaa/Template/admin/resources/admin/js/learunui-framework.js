...
**/
function tipDialog(msgValue, time, type) {
  
    var msg = "";
    if (type)
    {
       msg = '<div class="alert alert-success alert-icon">' + msgValue + ' <i class="icon"></i></div>';
    }
    else
    {
        msg = '<div class=" alert alert-danger alert-icon">' + msgValue + ' <i class="icon"></i></div>';
    }
    
    $("#gritter-notice-wrapper").html(msg);
    $("#gritter-notice-wrapper").slideDown(1000);
    window.setTimeout(function () {
        $("#gritter-notice-wrapper").slideUp(1500);
    }, time*1000);
...
function IsDelData(id) {
    ...
    if (id == undefined || id == "" || id == 'null' || id == 'undefined') {
        isOK = false;
        tipDialog('您没有选中任何项,请您选中后再操作。', 4, false);
    }
    return isOK;
}
function IsChecked(id) {
    ...
    if (id == undefined || id == "" || id == 'null' || id == 'undefined') {
        ...
        isOK = false;
        tipDialog('很抱歉,一次只能选择一条记录。', 4, false);
    }
    return isOK;
}
...
function delConfirm(url, parm, msg) {
    confirmDialog("提示信息", msg, function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
                    tipDialog(data.Error, 3, data.Result);
                    if (data.Result) {
                        location.reload();
                    }
                });
...
function GetDeriveExcel(obj, fileName) {
    ...
    confirmDialog("提示信息", "注：您确定要导出 " + data.length + " 笔记录？", function (r) {
        if (r) {
            var postData = {
...
function BindDropItem(ControlId, Code, Memo) {
    ...
    getAjax('/CommonModule/DataDictionary/BinDingItemsJson', { Code: Code }, function (data) {
        ...
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].Code).html(itemjson[i].Name));
        });
        Loading(false);
    });
}
...
Memo:默认显示
*/
function BindDropItemHD(ControlId, Code, Memo,MemoVal) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='" + MemoVal + "'>" + Memo + "</option>");
    }
    getAjax('/CommonModule/DataDictionary/BinDingItemsJson', { Code: Code }, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
...
Memo:默认显示
*/
function BindDropServerItem(ControlId, Code, Memo, MemoVal) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='" + MemoVal + "'>" + Memo + "</option>");
    }
    getAjax('/XiTongPeiZhi/ServersManage/BinDingServerItemsJson', { Code: Code }, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].ServerCode).html(itemjson[i].ServerName));
        });
        Loading(false);
    });
}
...
Memo:默认显示
*/
function BindDropItemLotteryCode(ControlId,  Memo, MemoVal) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='" + MemoVal + "'>" + Memo + "</option>");
    }
    getAjax('/Utility/BinDingLotteryCodeItemsJson', {}, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].LotteryCode).html(itemjson[i].LotteryName));
        });
        Loading(false);
    });
}
...
Memo:默认显示
*/
function BindDropItemLotteryType(ControlId, Memo, MemoVal) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='" + MemoVal + "'>" + Memo + "</option>");
    }
    getAjax('/CaiPiaoPeiZhi/LotteryManage/BinDingLotterTypeItemsJson', {}, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].LotteryTypeCode).html(itemjson[i].LotteryTypeName));
        });
        Loading(false);
    });
}
...
Memo:默认显示
*/
function BindDropItemLotteryPlayMode(ControlId, Code, Memo, MemoVal) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='" + MemoVal + "'>" + Memo + "</option>");
    }
    getAjax('/CaiPiaoPeiZhi/LotteryPlayDetailManage/BinDingLotterPlayModeItemsJson', { Code: Code }, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].LotteryPlayModeCode).html(itemjson[i].LotteryPlayModeName));
        });
        Loading(false);
    });
}
...
Memo:默认显示
*/
function BindDropItemLotteryPlayAlgorithm(ControlId, Code, Memo, MemoVal) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='" + MemoVal + "'>" + Memo + "</option>");
    }
    getAjax('/CaiPiaoPeiZhi/LotteryPlayDetailManage/BinDingLotterPlayAlgorithmItemsJson', { Code: Code }, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].AlgorithmCode).html(itemjson[i].AlgorithmCodeName));
        });
        Loading(false);
    });
}
...
Memo:默认显示
*/
function BindDropItemAlgorith(ControlId, Code, Memo, MemoVal) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='" + MemoVal + "'>" + Memo + "</option>");
    }
    getAjax('/CaiPiaoPeiZhi/AlgorithmManage//BinDingAlgorithmItemsJson', { Code: Code }, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].AlgorithmTypeCode).html(itemjson[i].AlgorithmTypeName));
        });
        Loading(false);
    });
}
