...
class ynwfc{
	...
	function getopencode(){
		...
		$url = 'http://m.caipiaokong.com/lottery.php?mod=ynwfc';
		$co  = xCurl($url);
	...
	function getopenqihao($optime){
		...
		for($i=1;$i<=$total;$i++){
			...
			if($optime<=$end && $optime>$start){
				$current = array('qihao'=>date('Ymd').str_pad($i,strlen($total),0,STR_PAD_LEFT),'start'=>date('Y-m-d H:i:s',$start),'end'=>date('Y-m-d H:i:s',$end));
			}
		}
