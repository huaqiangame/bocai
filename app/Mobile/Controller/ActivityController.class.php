...
//					if(($user['groupid']-$jiangli[0]['groupid'])>=2)$amount++;
	...
	public function activityList(){
        $member_info = $_SESSION['userinfo'];
        $this->assign('member_info',$member_info);
        $data =   M('activitie')->order('created_at desc')->select();
        if($data){
            foreach ($data as &$val){
               $val['time'] = strtotime($val['start_at'])?date('Y-m-d',strtotime($val['start_at'])).'至'.date('Y-m-d',strtotime($val['end_at'])):'长期有效';
            }
        }
        $this->assign('data',$data);
        $this->display();
    }
    public function activityDetail($id=null){
        $member_info = $_SESSION['userinfo'];
        $this->assign('member_info',$member_info);
        $activitie_info = M('activitie')->where(['id'=>$id])->find();
        $this->assign('activitie_info',$activitie_info);
        $this->display();
    }
    function activity_apply(){
        if(IS_AJAX){
            ...
            if($apply_id){
                echo json_encode(['msg'=>'你已经申请过了']);exit;
            }
            $data['activity_id'] = $activity_id;
