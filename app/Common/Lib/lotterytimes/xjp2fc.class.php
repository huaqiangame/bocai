...
class xjp2fc {
	function drawtimes(){
		...
		$_day = (($_start_t - strtotime('2008-11-23')) / 3600 / 24);
		$draws = array();
		for($i=1;$i<=$total;$i++){
		...
		if(!$nowqihao){
			$nowqihao = str_pad($total,3,0,STR_PAD_LEFT);// 660 * ($_day+1) +
			$actionNo = 660 * ($_day+1) + intval($nowqihao);
			...
			$actionNo = $total * $_day + intval($nowqihao) + 702646;
			$nowdraws = [
				'expect'  => $actionNo,
