	...
	function drawtimes(){
		...
		if(!$nowqihao){
			...
			$nowqihao = str_pad($nowqihao,3,0,STR_PAD_LEFT);
			            $unix_time = strtotime('-1 day', $nowtime);

			$nowdraws = [
				 'expect' => date('G') < 5?date('md',$unix_time).$nowqihao:date('md',$nowtime).$nowqihao,
			...
			if(intval($nowqihao)==1){
				...
				$predraws = [
					 'expect' => date('G') < 5?date('md',$unix_time).$preqihao:date('md',$nowtime).$preqihao,
