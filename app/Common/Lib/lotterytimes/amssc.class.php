...
class amssc {
	function drawtimes(){
		...
		$jgtime = $_expecttime;
		for($i=1;$i<=$total;$i++){
			$start = strtotime($openstart) + ($i-1)*$jgtime;
			$end = strtotime($openstart)+$i*$jgtime ;
			$draws[$i] = array('start'=>date('H:i:s',$start),'end'=>date('H:i:s',$end));
		}
        $draws[$total]['end'] = date('Y-m-d H:i:s',strtotime($draws[$total]['end'])+86400);
		...
		if(!$nowqihao){
			...
			if(intval($nowqihao)==1){
				$preqihao = str_pad($total,$_length,0,STR_PAD_LEFT);
				$nowexpecttime = strtotime($draws[$total]['start'])-86400;
				$predraws = [
