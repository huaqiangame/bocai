...
class ApijiekouController extends \Common\Controller\ApijiekouController {
	...
	function lotterytimes(){
		...
		if(!$lotteryname){
			echo jsonreturn(['sign'=>false,'message'=>'参数错误']);exit;
		}
...
/*		//验证用户名
	...
	function adduser(){
        ...
        $recreation_arr = ['live'=>'娱乐城直播','egame'=>'娱乐城电子','sport'=>'娱乐城体育','lottery'=>'娱乐城彩票','chess'=>'娱乐城棋牌'];
		...
		foreach ($recreation_arr as $index=>$v){
            if(I($index)>$recreation[$index] || I($index)<0){
                $return = ['sign'=>false,'message'=>$v."必须在0~{$recreation[$index]}之间"];
                echo json_encode($return, JSON_UNESCAPED_UNICODE);exit;
            }
        }
...
/*		//验证用户名
        ...
        try {
            ...
            if ($Result['id']) {
                $recreation_data = array();
                foreach ($recreation_arr as $index => $v) {
                    $recreation_data[$index] = I($index);
                }
                $recreation_data['member_id'] = $Result['id'];
                $recreation_data['create_date'] = time();
                $recreation_data['upd_date'] = time();
                M('recreation_fandian')->add($recreation_data);
                $Result['sql'] = M()->_sql();
            }
            M()->commit();
            echo jsonreturn($Result);
...
//		echo json_encode(['sign'=>true,'state'=>$state]);
    ...
    //检测支付是否成功
    function notify_jftpay(){
        $_api = new \Lib\api;
        $Result = $_api->sendHttpClient('Api/Pay/notify_jftpay');
        return $Result;
    }
