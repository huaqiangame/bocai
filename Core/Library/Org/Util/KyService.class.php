...
class KyService{
    ...
    function betrecord($startDate,$endDate,$pageIndex,$pageSize){
		$api=new  Biapi();
		return  $api->GetMerchantReport('KY', $startDate, $endDate, $TimeStamp, $pageIndex, $pageSize) ;
		$salt = strtolower(substr(md5($this->apiAccount),0,5));
...
$api=new  Biapi();
		$r= $api->zzmoney('KY',$username,'OUT',$credit);
		if ($r===true){
    ...
    function trans_in($username,$credit,$billno){
		$api=new  Biapi();
		$r= $api->zzmoney('KY',$username,'IN',$credit);
		if ($r===true){
			$ret=array('code'=>1,'msg'=>"Success");
		}else{
			$ret=array('code'=>-1,'msg'=>$r);
	...
	function login($username,$code=null){
		...
		$api=new  Biapi();
	$r= $api->loginbi('KY',$username,$code);
