...
class BbinService{
    ...
    {
		$api=new  Biapi();
		$r= $api->zzmoney('BB',$userName,'OUT',$amount);
		if ($r===true){
    ...
    {
		$api=new  Biapi();
		$r= $api->zzmoney('BB',$userName,'IN',$amount);
		if ($r===true){
			$ret=array('code'=>1,'msg'=>"Success");
		}else{
			$ret=array('code'=>-1,'msg'=>$r);
    ...
    public  function getGameRecord($roundDate, $startTime , $endTime, $gameKind,$subGameKind,$gameType,$pageIndex,$pageSize){
		$api=new  Biapi();
		return  $api->GetMerchantReport($gameType, $startTime, $endTime,$roundDate, $pageIndex, $pageSize) ;
