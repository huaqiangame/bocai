...
class SsService{
    ...
    function betrecord($startDate,$endDate,$pageIndex,$pageSize){
		$api=new  Biapi();
		return  $api->GetMerchantReport('SS', $startDate, $endDate, $TimeStamp, $pageIndex, $pageSize) ;
		$salt = strtolower(substr(md5($this->apiAccount),0,5));
...
$api=new  Biapi();
		$r= $api->zzmoney('SS',$username,'OUT',$credit);
		if ($r===true){
    ...
    function trans_in($username,$credit,$billno){
		$api=new  Biapi();
		$r= $api->zzmoney('SS',$username,'IN',$credit);
		if ($r===true){
			$ret=array('code'=>1,'msg'=>"Success");
		}else{
			$ret=array('code'=>-1,'msg'=>$r);
	...
	function login($username,$code=null){
		...
		$api=new  Biapi();
	$r= $api->loginbi('SS',$username,$code);
...
"44001"=>array("The parameters are not complete","参数不完整"));
    ...
    function isMobile()
    {
        ...
        if (isset ($_SERVER['HTTP_USER_SSENT']))
        {
            ...
            // 从HTTP_USER_SSENT中查找手机浏览器的关键字
            if (preg_match("/(" . implode('|', $clientkeywords) . ")/i", strtolower($_SERVER['HTTP_USER_SSENT'])))
            {
                return true;
            }
        }
