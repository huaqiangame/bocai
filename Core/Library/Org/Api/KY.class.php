...
class KY extends Des
{
    private $actype=0;//1表示 生产  0 测试
    private $balance_url = 'http://link.api.52miduo.com/api/KY2/GetBalance';
    private $login_url = 'http://link.api.52miduo.com/api/KY2/Login';
    private $trans_url= 'http://link.api.52miduo.com/api/KY2/Transfer';
    private $betRecord_url= 'http://link.api.52miduo.com/api/KY2/GetBetRecord';
        ...
        if($res['result']===1){
            if(is_array($res['data'])){
                ...
                foreach ($res['data'] as $idx=>$v){
                    $save_data[] = ['BillNo'=>$v['GameID'],'PlayerName'=>$v['Accounts'],
                        'BetTime'=>$v['WagersDate'],'BetAmount'=>$v['CellScore'],'NetAmount'=>$v['Profit'],
                        'GameCode'=>$v['KindID'],'TableCode'=>$v['TableID'],'ValidBetAmount'=>$v['CellScore'],
                        'CreateDate'=>date('Y-m-d H:i:s'),'GameType'=>'kY' ];
                }
