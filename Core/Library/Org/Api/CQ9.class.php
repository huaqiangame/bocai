...
class CQ9 extends Des{

    private $actype=1;//1表示 生产  0 测试
    private $balance_url = 'http://link.api.52miduo.com/api/CQ9/GetBalance';
    private $login_user = 'http://link.api.52miduo.com/api/CQ9/Login';
    private $trans_url= 'http://link.api.52miduo.com/api/CQ9/Transfer';
    private $betRecord_url= 'http://link.api.52miduo.com/api/CQ9/GetBetRecord';
            ...
            if(is_array($res['data']['bet_data'])&&!empty($res['data']['bet_data'])){
                ...
                foreach ($res['data']['bet_data'] as $idx=>$v){
                    $save_data[] = ['BillNo'=>$v['transaction_id'],'PlayerName'=>$v['player_name'],'GameCode'=>$v['gameCode'],
                        'NetAmount'=>$v['jackpot_win'],'BetTime'=>$v['time'],'BetAmount'=>$v['bet'],'ValidBetAmount'=>$v['bet'],
                        'Flag'=>$v['flag'],'TableCode'=>$v['round_id'],'RecalcuTime'=>$v['recalcuTime'],'BeforeCredit'=>$v['ending_balance'],
                        'game_name'=>$v['game_id'],'CreateDate'=>date('Y-m-d H:i:s'),'DeviceType'=>$v['deviceType']?$v['deviceType']:$device_type,
                        'GameType'=>'CQ9','result'=>$v['Result'],'GameKind'=>$v['game_type']=='chess'];
                }
                if($save_data){
