...
class SUNBET extends Des
{
    private $actype=0;//1表示 生产  0 测试
    private $balance_url = 'http://link.api.52miduo.com/api/SUNBET/GetBalance';
    private $login_url = 'http://link.api.52miduo.com/api/SUNBET/Login';
    private $trans_url= 'http://link.api.52miduo.com/api/SUNBET/Transfer';
    private $betRecord_url= 'http://link.api.52miduo.com/api/SUNBET/GetBetRecord';
            ...
            if(is_array($res['data'])){
                ...
                foreach ($res['data'] as $idx=>$v){
                    $save_data[] = ['BillNo'=>$v['transaction_id'],'PlayerName'=>$v['player_name'],'BetTime'=>$v['time'],'BetAmount'=>$v['bet'],
                        ...
                        ,'CreateDate'=>date('Y-m-d H:i:s'),'GameType'=>'SUNBET'];
                }
